...
@export_range(0.0, 1.0) var grain_amount: float = 1.0
...
func _initialize_compute() -> void:
	...
	# Compile our shader.
	var shader_file := load("res://addons/FilmGrain/film_grain.glsl")
	...
	if rd and p_effect_callback_type == EFFECT_CALLBACK_TYPE_POST_TRANSPARENT and pipeline.is_valid():
		...
		if render_scene_buffers:
			...
			var push_constant := PackedFloat32Array([
				...
				size.y,
				grain_amount,
				float(Time.get_ticks_usec() % 1_000_000) / 1_000_000.0,
			])
